...
export const CalendarView: React.FC<CalendarViewProps> = ({ instruments, bookings }) => {
    ...
    return (
      <div className="grid grid-cols-7 border-t border-l border-gray-300 dark:border-gray-600">
        {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(day => (
          <div key={day} className="text-center font-bold p-2 border-b border-r border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-700">{day}</div>
        ))}
        {days.map((day, index) => (
          <div key={index} className="h-32 border-b border-r border-gray-300 dark:border-gray-600 p-1 overflow-y-auto">
            ...
            {day && bookings
              ...
              .map(booking => {
                  ...
                  return (
                    <div key={booking.id} className={`p-1 text-xs rounded mt-1 ${getInstrumentColor(booking.instrumentId, instruments)}`}>
                      <p className="font-bold">{instrument?.name}</p>
                      <p>{new Date(booking.startTime).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})} - {new Date(booking.endTime).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</p>
    ...
    return (
        <div className="flex flex-col">
            <div className="grid grid-cols-7">
                {weekDays.map(day => (
                    <div key={day.toISOString()} className="text-center font-bold p-2 border-b border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-700">
            ...
            <div className="grid grid-cols-7 h-[60vh] overflow-y-auto border-l border-gray-300 dark:border-gray-600">
                {weekDays.map(day => (
                    <div key={day.toISOString()} className="border-r border-gray-300 dark:border-gray-600 p-1">
                        {bookings
                          .filter(b => new Date(b.startTime).toDateString() === day.toDateString())
                          .map(booking => {
                              const instrument = instruments.find(i => i.id === booking.instrumentId);
                              return (
                                <div key={booking.id} className={`p-2 my-1 text-sm rounded ${getInstrumentColor(booking.instrumentId, instruments)}`}>
                                    <p className="font-bold">{instrument?.name}</p>
                                    <p className="text-xs">{new Date(booking.startTime).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})} - {new Date(booking.endTime).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</p>
                                </div>
                              );
                          })}
                    </div>
  ...
  // A simplified day view would go here. For brevity, week and month are shown.
    ...
    <div className="bg-white/50 dark:bg-gray-800/50 backdrop-blur-sm p-6 rounded-lg shadow-md border border-white/20 dark:border-gray-700/50">
      <div className="flex justify-between items-center mb-4">
        <div className="flex items-center space-x-2">
            <button onClick={handlePrev} className="px-2 py-1 rounded-md hover:bg-gray-200 dark:hover:bg-gray-700">&lt;</button>
            <h2 className="text-xl font-bold">
                {view === 'month' && currentDate.toLocaleDateString([], { month: 'long', year: 'numeric' })}
                {view === 'week' && `Week of ${currentDate.toLocaleDateString([], { month: 'short', day: 'numeric' })}`}
            </h2>
            <button onClick={handleNext} className="px-2 py-1 rounded-md hover:bg-gray-200 dark:hover:bg-gray-700">&gt;</button>
        ...
        <div>
            <select value={view} onChange={e => setView(e.target.value as any)} className="px-3 py-1 bg-white/50 dark:bg-gray-700/50 border border-gray-300 dark:border-gray-600 rounded-md">
